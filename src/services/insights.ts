import type { OrderAggregate, OrderItem, ProductCategory, ReturnRecord, ReturnRequest } from "../types/order";
import { INVESTMENT_ANNUAL_RATE, PRIME_SHIPPING_COST_EUR } from "../utils/constants";
...
export function calculateInsights({ items, orders, returns, returnRequests }: InsightsInput): Insights {
  ...
  if (firstOrderDate && lastOrderDate) {
    const diff = Math.max(1, Math.round((lastOrderDate.getTime() - firstOrderDate.getTime()) / MS_PER_DAY) + 1);
    daysActive = diff;
  ...
  return {
    ...
    investmentOpportunityCost: calculateInvestmentOpportunityCost(items, INVESTMENT_ANNUAL_RATE),
    primeSavingsEstimate: calculatePrimeSavingsEstimate(orders, PRIME_SHIPPING_COST_EUR),
